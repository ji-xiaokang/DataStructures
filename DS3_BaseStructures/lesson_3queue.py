'''
    队列:是一系列有顺序的元素的结合，新元素加如在队列的一端，这一端叫做“队尾(rear)”
        已有元素的移出发生在队列的另一端，叫做“队首(front)”。当一个元素被加入到队列之后，他就从队尾向队首前进，直到他成为下一个即将被移除队列的元素
    先进先出(FIFO):最新被加入的元素处于队尾，在队列中停留最长时间的是元素处于队首
    ----------------
rear                front
    ----------------
    ...
    抽象数据类型(ADT):
        ...
        dequeue()  从队首移出数据项，无序参数，返回值为队首数据项
...
'''
    ...
    总共10*2 = 20次打印任务，平均没3分钟产生一个打印任务
    在3分钟内的任意一秒产生一个打印任务的概率是task/180，随机数模拟：如果生成的随机数是180，就可以认为生成了一个任务

    过程：
        1.创建一个空打印任务队列，每个任务在生成是被赋予一个“时间戳”
        2.一个小时中的每一秒(currentSecond)都需要哦按段；
          是否有新的打印任务生成，如果有，把他加入打印队列；
          如果打印机空闲并且队列不为空；
          1.从队列中拿出一个任务交给打印机
          2.从加入打印机时间减去 - 加入队列的时间 = 等待时间
          3.将该任务的等待时间加入到一个列表中，方便后续时候，计算总的学生打印花费的时间
          4.基于打印的页数的随机数，求出需要多长时间打印
        3.打印机工作中，那对于打印机而言，就是工作了一秒：对于打印任务而言，他离打印结束又近了一秒
        4.打印任务完成，剩下时间为0，打印机进入空闲状态
     Python实现：
