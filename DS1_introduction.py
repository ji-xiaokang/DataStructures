...
'''
    ...
    4.最终算法用什么去衡量？
      时间复杂度
    5.表示法：大O记法
      假设计算机执行算法每个基本操作的时间是固定的一个时间单位，那么有多少个基本操作就代表会花费多少时间
      虽然对于不同的机器环境而言，确切的时间单位是不同的，但是对于算法进行多少个基本操作在规模数量级上是相同的
      因此，可以忽律机器环境的影响而客观的反应算法的时间效率
      对于算法的时间效率，用“大O记法”
      O(n^3)  100n^2  1000n^2
      O(n^2)
    6.时间复杂度分类
      最优时间复杂度：算法完成工作最少需要多少基本操作（过于理想化）
      最坏时间复杂度：。。。。最多。。（提供了一种保证，表名算法在此程度的基本操作中一定能完成工作）
      平均时间复杂度：。。。。平均。。（对算法整体一个全面的评价，但是这种衡量凡是没有保证）
...
      循环结构，时间复杂度按乘法进行计算
      分支：取最大值
      判断一个算法的效率时，只需要关注操作数量的最高次数项，其他次要项和常数项可以忽略
      没有特殊情况下，可以分析的都是最坏时间复杂度
    8.练习
      12                                O(1)
      2n+3                              O(n)                      
      3n^ + 2n +1                       O(n^2)
